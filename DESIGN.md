...
| D10 | Menus site on this repository's own GitHub Pages (`thodha.github.io/cafe`): a workflow checks out the public `ThoDHa/recipes` repository, runs the generator's tests, generates the drinks menu from `cafe.md` via the stdlib generator and templates in `site/`, publishes it as the homepage (`index.html`, `menu.html`) alongside the hand-authored `kitchen.html` and `bar.html`, and deploys on every push to `main`; a print-budget pass (weasyprint, font-size stepping 16px down to an 11px floor) keeps the menu, kitchen, and bar pages within two printed A4 and Letter pages and the compact menu (`menu/compact.html`) within one, failing the build otherwise; the repository is public to use Pages | Hosting on the recipes repo's Pages: cross-repo pushes need a PAT secret and the URLs live under `/recipes/`; publishing hand-authored pages only: the public menu drifts from the recipes as they evolve; generating from `menu.json`: couples the public site to the ordering pipeline, which must track the recipes instead |
...
│   ├── menu_source.py         # shared stdlib parsing core for recipes cafe.md (site + ordering)
...
│   ├── templates/           # drinks page templates: full (menu.html) and one-page compact
