...
| D10 | Menus site on this repository's own GitHub Pages (`thodha.github.io/cafe`): a workflow checks out the private `ThoDHa/recipes` repository with the `RECIPES_TOKEN` PAT secret, runs the generator's tests, generates the drinks menu from `cafe.md`, the bar menu from `cocktails.md`, the kitchen menu from the recipes README index over the dish files, and the pantry page from `cafe_pantry.md`, via the stdlib generator and templates in `site/`, publishes the drinks page as the homepage (`index.html`, `menu.html`) alongside the generated `bar.html`, `kitchen.html`, and `pantry.html`, and deploys on every push to `main`; a print-budget pass (weasyprint, font-size stepping 16px down to an 11px floor) keeps the menu, kitchen, and pantry pages within two printed A4 and Letter pages and the bar and compact menus within one, failing the build otherwise; the same pass renders each menu to a print-ready PDF next to its HTML (`menu.pdf`, `menu/compact.pdf`, `bar.pdf`, `kitchen.pdf`, `pantry.pdf`, weasyprint on A4 sheets, gated by the same page budgets), whose `@page` bottom margin box prints the brand line on every sheet inside the reserved bottom margin band, the every-page printed footer browsers cannot offer in their print preview, so the PDF is the print path while the HTML pages keep the in-flow footer at the content end and link their PDF with a screen-only "PDF View" link; the repository is public to use Pages. Superseded 2026-09-08: the runtime browser print scaler (print-fit.js, added after this decision) is removed from the site; every browser print uses the build-injected margin root, re-verified on A4 and Letter by make verify-print-chrome, and the print-ready PDFs remain the print path | Hosting on the recipes repo's Pages: cross-repo pushes need a PAT secret and the URLs live under `/recipes/`; publishing hand-authored pages only: the public menu drifts from the recipes as they evolve; generating from `menu.json`: couples the public site to the ordering pipeline, which must track the recipes instead |
...
│   ├── menu_source.py         # shared stdlib parsing core for recipes cafe.md, cocktails.md, the kitchen index, and cafe_pantry.md (site + ordering)
...
**Public menus site (D10, FR-12):** the site at `thodha.github.io/cafe` is generated by the deploy, independent of the home server. The workflow checks out both repositories, and `site/generate.py` (stdlib only) parses `recipes/cafe.md`: the four drink sections map to Cà Phê, Trà, Mát-cha, and Giải Khát (in that display order), and the cold-foam builds become Kem; each category's blurb paragraph under its `##` heading becomes the section note. Items are derived entirely from the file, so a drink added to recipes appears at the next deploy with no generator change; only menu-shape knowledge recipes cannot express (section titles, ten Vietnamese names, the Hot Tea temperature the prose cannot prove, three foam descriptions) lives in the generator's configuration. An unrecognized drink-like section raises `UnmappedSectionError` and fails the build loudly. Temperatures derive conservatively from explicit cues ("iced only", "hot or iced", paired Hot/Iced blocks, ice ingredients excluding ice cream), with a test cross-checking every derived temperature against `menu.json`. The drinks menu renders through templates carrying the house drinks-menu CSS and chrome (items filling columns top-down) with all ordering artifacts stripped; it is written to both `index.html` and `menu.html`, so it doubles as the homepage; the customer-facing drinks pair renders unpriced, with the priced views living under the Giá family. Three further pages are produced: the compact menu (`compact.html`, served at `/cafe/menu/`), a one-page print reference keeping names, nóng/đá tags, section notes, and each drink's description, with the last set small, muted, and italic (Lora at 0.75rem on screen, 0.65rem with tight leading in print) so the denser three-column layout still fits its page budget; the priced customer menu (`prices/menu.html`); and the generated `kitchen.html`, each carrying a back link to the drinks menu. The priced pages join the same checkout's cost workbook through the same loud join: the priced customer menu mirrors the drinks menu's layout and print shape with one selling price right-justified on each non-Kem drink's name line (`prices/menu.pdf` beside it), and the cost-reference pair (`prices.html`, `prices/compact.html`) shows each drink's planning cost beside the suggested retail; the main drinks pair and the compact menu carry no prices. The bar menu (`bar.html`) is generated from `cocktails.md`: the `##` template families become the COCKTAILS section with the first paragraph under each heading as its description. The kitchen menu (`kitchen.html`) is generated from the same checkout's README index: the "Available Recipes" groups map to the five sections, each top-level bullet becomes a dish whose display name defaults to the link text, the description falls through the README one-liner to the dish file's intro paragraph, variant bullets merge into single items where the config says so (Asparagus, Broccolini), and every referenced file and `#anchor` is verified against the tree with build.py's unicode-preserving GitHub slugs. The curated Vietnamese-led names, condensed sauce names, section note, and menu-voice descriptions for both pages live in `site/menu-overrides.json`, so each generated page is byte-identical to its former hand copy; a new family or dish appears at the next deploy with no config change, while a stale override, a missing anchor, or an unmapped index group fails the build. The pantry page (`pantry.html`) is generated from `cafe_pantry.md`: the `##` buying groups become sections under curated Vietnamese leads held in the generator (a group the list adds later falls back to its English title), each `- Name (buy spec): what it buys for` bullet becomes an item with the buy spec in the subtitle slot, the file's intro and trailing paragraphs become page notes, and its two-page print budget matches the kitchen's. Every page then passes the print-budget pass: weasyprint renders it at a stepped print root font size, 16px down to an 11px floor via 1px steps on Letter (binding) with a final A4 check, until the menu, priced menu, kitchen, prices, and pantry fit two pages and the bar, compact, and prices-compact menus fit one; `PrintFitError` is raised and the build fails when the floor cannot satisfy the budget. The repository is public, which Pages on the free plan requires; a secret scan preceded the visibility change.
...
`menu/ordering-overrides.json` plus the recipes repository's `cafe.md` are the sources of truth for the orderable menu. The recipes file yields the drinks: the four drink sections map to Cà Phê, Trà, Mát-cha, and Giải Khát, the cold-foam builds become Kem, and temperatures derive conservatively from explicit cues ("iced only", "hot or iced", paired Hot/Iced blocks, ice ingredients excluding ice cream) through the shared stdlib parser in `menu/menu_source.py` (also powering the public site, so the two derivations cannot diverge). The checked-in `menu/ordering-overrides.json` supplies version, orderRules, categories (the five drink sections), and modifierGroups, plus per-drink overrides keyed by recipes drink name for what recipes cannot express: ids, display names, menu copy, images, and modifier rules. `menu/menu.json` is a generated artifact: the server's `generate-menu` console script (wired like the existing `export-openapi` entry in `server/pyproject.toml`, plus jsonschema validation against `menu/menu.schema.json`) derives it on demand via the root `make menu` target; hand-editing `menu.json` is prohibited. A drink added to recipes appears with sensible defaults (slug id, no image, sweetness plus cold foam when iced; the kem builds carry none) and no code or config change. Items carry `id`, `name` (English), `nameVi`, `description`, `categoryId`, `temperatures`, `modifierGroupIds`, and optional `imagePath` (`/images/menu/<itemId>.<ext>`, files in `menu/assets/`, absent means placeholder). Modifier groups encode the PRD's frozen customization model; per-drink rules live in the overrides config (example: Matcha Sữa milk options differ hot versus iced; cold foam attaches only to iced). pytest validates `menu.json` against `menu.schema.json`: referential integrity of modifier groups, non-empty temperatures, valid defaults, path convention on `imagePath`. A staleness test regenerates the menu from `cafe.md` plus the overrides config and asserts it matches the committed `menu.json`, so the sources and the artifact cannot drift.

**Deferred:** runtime auto-sync from GitHub. When wanted, the server will poll `https://raw.githubusercontent.com/ThoDHa/recipes/main/cafe.md` with conditional GET (ETag) on an interval, regenerate and validate, hot-swap, and broadcast a refresh over the existing SSE event hub; deferred because generation on demand is sufficient for now.
...
**Design tokens:** the house drinks-menu palette and type stack (originally set by the retired print menu page) preserved verbatim in `design/tokens.css` (cream and cobalt, Bungee, Lora, Be Vietnam Pro, nóng/đá pill styles), one token set for both views.
...
| Public menus site | push to `main` (or workflow dispatch) | GitHub Actions checks out cafe plus recipes, runs `pytest site/` under weasyprint, `site/generate.py` builds and print-fits `site/public/`, Pages deploys it at `thodha.github.io/cafe` (D10, FR-12); static pages only, no server, no ordering |
...
| 2 | CP-2 Menu Pages | `site/` generator, templates, print-fit pass and tests; `.github/workflows/menu-pages.yml`; Pages enabled (`build_type: workflow`), repository public | Site test suite green; push to `main` deploys; the drinks menu is the homepage, all four pages reachable at `thodha.github.io/cafe`, the drinks menu carries no ordering artifacts, and each page stays within its print budget (two pages, one for the compact) |
...
| Menu generation from recipes or overrides config produces an invalid menu | Medium | Config validation at build time (unknown drinks, unknown groups, missing descriptions) plus schema validation and three manual spot checks (temperature-dependent milk, iced-only drink, foam-as-drink) at CP-1 |
...
| Ordering display drifts from the recipes (menu copy, names, temperatures) | Medium | The shared parsing core powers both the public site and the ordering menu, and the staleness test regenerates `menu.json` from `cafe.md` plus the overrides config in CI, failing loudly on drift; ordering-only enrichment is confined to the overrides config |
