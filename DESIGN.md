...
| D10 | Menus site on this repository's own GitHub Pages (`thodha.github.io/cafe`): a workflow checks out the private `ThoDHa/recipes` repository with the `RECIPES_TOKEN` PAT secret, runs the generator's tests, generates the drinks menu from `cafe.md`, the bar menu from `cocktails.md`, and the kitchen menu from the recipes README index over the dish files, via the stdlib generator and templates in `site/`, publishes the drinks page as the homepage (`index.html`, `menu.html`) alongside the generated `bar.html` and `kitchen.html`, and deploys on every push to `main`; a print-budget pass (weasyprint, font-size stepping 16px down to an 11px floor) keeps the menu and kitchen pages within two printed A4 and Letter pages and the bar and compact menus within one, failing the build otherwise; the same pass renders each menu to a print-ready PDF next to its HTML (`menu.pdf`, `menu/compact.pdf`, `bar.pdf`, `kitchen.pdf`, weasyprint on A4 sheets, gated by the same page budgets), whose `@page` bottom margin box prints the brand line on every sheet inside the reserved bottom margin band, the every-page printed footer browsers cannot offer in their print preview, so the PDF is the print path while the HTML pages keep the in-flow footer at the content end and link their PDF with a screen-only "PDF View" link; the repository is public to use Pages. Superseded 2026-09-08: the runtime browser print scaler (print-fit.js, added after this decision) is removed from the site; every browser print uses the build-injected margin root, re-verified on A4 and Letter by make verify-print-chrome, and the print-ready PDFs remain the print path | Hosting on the recipes repo's Pages: cross-repo pushes need a PAT secret and the URLs live under `/recipes/`; publishing hand-authored pages only: the public menu drifts from the recipes as they evolve; generating from `menu.json`: couples the public site to the ordering pipeline, which must track the recipes instead |
...
│   ├── menu_source.py         # shared stdlib parsing core for recipes cafe.md, cocktails.md, and the kitchen index (site + ordering)
...
│   ├── generate.py          # stdlib generator: parses recipes cafe.md and cocktails.md, renders templates
