...
- **Print budgets enforced at build time**: the generator renders every page with weasyprint and steps the print font size down (16px to an 11px floor) until the menu and kitchen pages fit two printed pages on A4 and Letter, and the bar and compact pages fit one. A page that cannot fit fails the build instead of shipping an overflowing printout.
